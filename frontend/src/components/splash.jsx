...
const Splash = () => {
  return (
    <div className="empty-state-container" style={{ position: 'relative', height: '100%', width: '100%', overflow: 'hidden', background: '#010102' }}>
      ...
      {/* HEADER HUD */}
      <div style={{ position: 'absolute', top: '30px', right: '40px', color: '#00f2ff', fontFamily: 'monospace', fontSize: '16px', letterSpacing: '3px', zIndex: 15, opacity: 0.7 }}>
        USER_UPLINK: DR_LINDSAY // AUTH: DIRECTOR
      </div>

      <div style={{ position: 'absolute', top: '7%', width: '100%', textAlign: 'center', zIndex: 15 }}>
        <span className="dynamic-title" style={{ fontSize: '55px', fontWeight: '900', letterSpacing: '18px', fontFamily: '"Arial Black", sans-serif', color: '#e0ffff', textShadow: '0 0 20px rgba(0, 242, 255, 0.5)' }}>THE COUNCIL</span><br/>
        <span className="neon-ghost-title" style={{ animation: 'neon-flicker 5s infinite', color: '#00f2ff', fontSize: '28px', fontWeight: 'bold', letterSpacing: '8px', fontFamily: 'monospace' }}>NEON_GHOST</span>
      </div>

      {/* SCI-FI DATA STREAMS (CORNERS) */}
      <div style={{ position: 'absolute', top: '30px', left: '40px', color: '#00f2ff', fontFamily: 'monospace', fontSize: '12px', opacity: 0.4, lineHeight: '1.5', zIndex: 15 }}>
      ...
      <div style={{ position: 'absolute', bottom: '8%', width: '100%', textAlign: 'center', zIndex: 15 }}>
        <span className="sigma-glitch-text" style={{ animation: 'cyber-glitch 4s infinite', color: '#ff3e3e', fontSize: '28px', fontWeight: 'bold', letterSpacing: '4px' }}>PRIORITY: SIGMA // ACCESS: CLASSIFIED</span>
      </div>

      <div style={{ position: 'absolute', bottom: '8%', left: '40px', color: '#00ff41', fontFamily: 'monospace', fontSize: '24px', letterSpacing: '2px', textShadow: '0 0 10px rgba(0,255,65,0.4)', zIndex: 15, animation: 'subtle-star-pulse 3s infinite' }}>
        STATUS: ONLINE
      </div>

      <div style={{ position: 'absolute', bottom: '8%', right: '40px', color: '#e0e0e0', fontFamily: 'monospace', fontSize: '22px', letterSpacing: '2px', zIndex: 15, display: 'flex', alignItems: 'center', gap: '10px' }}>
